...
class NewSalesCartList extends StatefulWidget {
  final CustomerDataModel customer;
  final List<NewSalesCart>? cartProductList;

  const NewSalesCartList({Key? key,this.cartProductList, required this.customer}) : super(key: key);



  @override
  State<NewSalesCartList> createState() => _NewSalesCartListState();
}

class _NewSalesCartListState extends State<NewSalesCartList> {
                            ...
                            children: [
                              ...
                              Text(
                                "Price: Ksh. ${widget.customer.customerGroup=="Wholesaler"
                                    ?widget.cartProductList![ind].productMo!.wholesalePrice
                                    :widget.customer.customerGroup=="Distributor"
                                    ?widget.cartProductList![ind].productMo!.distributorPrice
                                    :widget.cartProductList![ind].productMo!.retailPrice!}",
                                style: Styles.smallGreyText(context).copyWith(fontWeight: FontWeight.w600, color: Colors.grey),
                              ),
                              Text(
                                "Total: Ksh. ${(int.parse(widget.customer.customerGroup=="Wholesaler"
                                    ?widget.cartProductList![ind].productMo!.wholesalePrice!
                                    :widget.customer.customerGroup=="Distributor"
                                    ?widget.cartProductList![ind].productMo!.distributorPrice!.toString()
                                    :widget.cartProductList![ind].productMo!.retailPrice!)) * (widget.cartProductList![ind].qty!)}",
                            ...
                            children: <Widget>[
                              widget.cartProductList![ind].qty !=0?  IconButton(icon: Icon(Icons.remove),onPressed: (){
                                var data = NewSalesCart(
                                    ...
                                    price: widget.cartProductList![ind].price

                                );
                                _qtycontrollers![ind].text = (widget.cartProductList![ind].qty! - 1).toString();
                                cartcontroller.addNewSalesCart(data);
