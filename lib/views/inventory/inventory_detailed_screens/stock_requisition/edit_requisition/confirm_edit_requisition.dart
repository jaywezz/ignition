...
class ConfirmEditRequisition extends ConsumerStatefulWidget {
  final RequisitionModel requisition;
  const ConfirmEditRequisition({Key? key, required this.requisition}) : super(key: key);

  @override
  ConsumerState<ConfirmEditRequisition> createState() => _ConfirmEditRequisitionState();
}

class _ConfirmEditRequisitionState extends ConsumerState<ConfirmEditRequisition> with SingleTickerProviderStateMixin{
  int _currentIndex = 0;
          ...
          child: Column(
            children: [
              ref.watch(addStockLiftNotifier).isLoading?CircularProgressIndicator():FullWidthButton(
                action: () async{
                  await ref.read(addStockLiftNotifier.notifier).editStockRequisition(widget.requisition.id!).then((value) => Get.close(2));
